...
USER_BOT_WARN_ZERO = "**Stai spammando troppo e sei stato bloccato dal bot."
USER_BOT_NO_WARN = ("[SYSTEM SECURITY BOT](tg://user?id=961408537)\n\n"
                    "**Specifica qui sotto cosa ti serve**"
                    f"{DEFAULTUSER} \n\n"
                    "**Lascia il tuo messaggio, rispondo entro 24h **\n\n"
    ...
    async def approve_p_m(event):
        ...
        if len(approved_users) > 0:
            for a_user in approved_users:
                if a_user.reason:
                    APPROVED_PMs += f"ðﾟﾑﾉ [{a_user.chat_id}](tg://user?id={a_user.chat_id}) for {a_user.reason}\n"
                else:
                    APPROVED_PMs += f"ðﾟﾑﾉ [{a_user.chat_id}](tg://user?id={a_user.chat_id})\n"
        else:
